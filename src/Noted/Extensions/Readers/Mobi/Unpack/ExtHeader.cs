...
namespace XRayBuilder.Core.Unpack.Mobi
{
    ...
    public sealed class ExtHeader
    {
        ...
        public ExtHeader(EndianBinaryReader reader)
        {
            ...
            if (identifier != "EXTH")
            {
                throw new Exception($"Invalid EXTH identifier: {identifier}");
            }
            ...
            {
                this.RecordList.Add(new ExtHRecord(reader));
            }

            reader.Seek(this.GetPaddingSize(DataSize), SeekOrigin.Current); // Skip padding bytes
        }
